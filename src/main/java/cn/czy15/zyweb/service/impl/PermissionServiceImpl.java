...
import cn.czy15.zyweb.mapper.SysPermissionMapper;
...
public class PermissionServiceImpl implements PermissionService {
    ...
    @Override
    public List<PermissionDetailRespNodeVO> selectAllByDetail() {
        return getTrees(selectAll());
    }
    ...
    // 遍历递归所有数据【详细】
    private List<PermissionDetailRespNodeVO> getTrees(List<SysPermission> all) {

        List<PermissionDetailRespNodeVO> list = new ArrayList<>();
        if (all == null || all.isEmpty()) {
            return list;
        }
        for (SysPermission sysPermission : all) {
            if (sysPermission.getPid().equals("0")) {
                PermissionDetailRespNodeVO respNodeVO = new PermissionDetailRespNodeVO();
                BeanUtils.copyProperties(sysPermission, respNodeVO);
                respNodeVO.setTitle(sysPermission.getName());
                respNodeVO.setChildren(getChilds(sysPermission.getId(), all));
                list.add(respNodeVO);
            }
        }
        return list;
    ...
    private List<PermissionDetailRespNodeVO> getChilds(String id, List<SysPermission> all) {
        List<PermissionDetailRespNodeVO> list = new ArrayList<>();
        for (SysPermission s :
                all) {
            if (s.getPid().equals(id)) {
